...
class DinoPatchDistillTrainer(DetectionTrainer):
    loss_names = ("box_loss", "cls_loss", "dfl_loss", "dino_patch_loss")
    ...
    def get_model(self, cfg: str | None = None, weights: str | None = None, verbose: bool = True) -> DetectionModel:
        ...
        model.dino_patch_projector = nn.Conv2d(
    ...
    def _distill_loss(self, model: DetectionModel, batch: dict[str, torch.Tensor], preds: torch.Tensor | None = None):
        ...
        if not isinstance(student_feature, torch.Tensor):
            zero_patch_loss = detection_loss.new_zeros(1)
            return detection_loss, torch.cat((detection_items, zero_patch_loss))
        grid = self.distill_settings.teacher_patch_grid
        pooled = torch_functional.adaptive_avg_pool2d(student_feature, (grid, grid))
        projected = model.dino_patch_projector(pooled).flatten(2).transpose(1, 2)
        teacher_tokens = self._teacher_patch_tokens(batch["img"])
        patch_loss = 1.0 - torch_functional.cosine_similarity(projected, teacher_tokens, dim=2).mean()
        total_loss = detection_loss + patch_loss * self.distill_settings.lambda_patch
        loss_items = torch.cat((detection_items, patch_loss.detach().reshape(1)))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/experiments/dinov3_patch_distill_visdrone_10pct.yaml")
...
def settings_from_config(path: Path, args: argparse.Namespace) -> TrainSettings:
    ...
    return TrainSettings(
        ...
        distill=DistillSettings(
            ...
            lambda_patch=float(distill["lambda_patch"]),
        ),
    )
...
def main() -> None:
    ...
    trainer = DinoPatchDistillTrainer(settings=settings, overrides=trainer_overrides(settings))
    trainer.train()
